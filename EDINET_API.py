...
def datelog():
    ...
    if os.path.exists('datelog.pkl') :
        with open('datelog.pkl','rb') as log_file:
            datelogs=pickle.load(log_file)
    else : datelogs=[]
    datelogs.sort()
...
def json_docs(dt_5y,dt_today,df_doc1) :    
    ...
    while(dt_5y < dt_today):
        ...
        if sdt not in datelogs : #過去に読み込んだ日付は読み込まない
            ...
            jsons=rjson['results']              
            if  jsons!=[] : doc_list.append(jsons)            
...
if __name__=='__main__':
    last_day=date.today()
    start_day=last_day-timedelta(days=2) #約5年前なら5*365　正確さを求めるならrelativedelta
